...
@section('title')
    Master Tematik Inovasi
@endsection

@section('title-desc')
    Daftar Tematik untuk Data Inovasi
@endsection

@section('buttons')
    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalPopup"
        onclick="modal(0, 'tematik')">Tambah Data</button>
@endsection
                            ...
                            <tr>
                                ...
                                <td>
                                    <button data-target="#modalPopup" data-toggle="modal"
                                        onclick="modal({{ $item->id }}, 'tematik')"
                                        class="btn m-1 btn-block btn-sm btn-warning"><i
                                            class="fa fa-edit"></i>&nbsp;&nbsp;Edit</button>
                                    <form style="all: unset"
                                        action="{{ route('master.tematik.delete', ['id' => $item->id]) }}" method="post">
                                        @csrf
