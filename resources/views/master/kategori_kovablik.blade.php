...
@section('title')
    Master Kategori Kovablik
@endsection

@section('title-desc')
    Daftar Kategori untuk Data Kovablik
@endsection

@section('buttons')
    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalPopup"
        onclick="modal(0, 'kategori_kovablik')">Tambah Data</button>
@endsection
                            ...
                            <tr>
                                ...
                                <td>
                                    <button data-target="#modalPopup" data-toggle="modal"
                                        onclick="modal({{ $item->id }}, 'kategori_kovablik')"
                                        class="btn m-1 btn-block btn-sm btn-warning"><i
                                            class="fa fa-edit"></i>&nbsp;&nbsp;Edit</button>
                                    <form style="all: unset"
                                        action="{{ route('master.kategori_kovablik.delete', ['id' => $item->id]) }}" method="post">
                                        @csrf
