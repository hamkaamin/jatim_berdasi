            ...
            @if ($data != null && $data->status == 0)
                @if ($fase && $fase->active == 1 && strtotime($fase->tgl_berakhir) >= strtotime(date('Y-m-d H:i:s')))
                    @if (env('APP_NAME') == 'INOVASI DAERAH')
                        <button style="display: none" type="submit" class="btn btn-primary" name="status" value="1">Kirim
                            Inovasi</button>
                    @else
                        <button type="submit" class="btn btn-primary" name="status" value="1">Kirim
                            Inovasi</button>
                    @endif
                @else
