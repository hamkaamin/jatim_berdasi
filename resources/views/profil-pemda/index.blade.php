                        ...
                        <thead class="thead-light">
                            ...
                            @foreach ($provinsi as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->name }}</td>
                                    <td>
                                        <a href="{{ route('profil-pemda.detail', ['id' => $item->id]) }}" class="btn m-1 btn-block btn-sm btn-warning"><i class="fa fa-edit"></i>&nbsp;&nbsp; Detail</a>
                                    </td>
                                </tr>
                            @endforeach
                        <tbody>
        ...
        <div class="row">
            <div class="col">
                ...
                <form action="{{ route('profil-pemda.upload-pakta') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row my-2">
                        <div class="col-auto d-flex align-items-center"><label class="m-0 p-0">Upload Pakta Integritas</label></div>
                        <div class="col">
                            ...
                            @if (Auth::user()->pakta_integritas != null && file_exists(public_path('/pakta_integritas/'.Auth::user()->pakta_integritas)))
                                <a target="_blank" href="{{ asset('pakta_integritas/'.Auth::user()->pakta_integritas) }}">Download Pakta Integritas</a>
                            @endif
                        </div>
                    </div>
                </form>
